...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const deploymentFee = await deployer.estimateDeployFee(paymasterArtifact, [NFT_COLLECTION_ADDRESS]);
  const parsedFee = ethers.utils.formatEther(deploymentFee.toString());
  console.log(`The deployment is estimated to cost ${parsedFee} ETH`);
  // Deploy the contract
  const paymaster = await deployer.deploy(paymasterArtifact, [NFT_COLLECTION_ADDRESS]);
  ...
  const contractFullyQualifedName = "contracts/paymasters/ERC721gatedPaymaster.sol:ERC721gatedPaymaster";
